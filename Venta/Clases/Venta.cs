...
namespace Venta.Clases
{
    ...
    class Venta
    {
        ...
        /* private int cod_Vtemp()
         {
             int id;
             DataTable datos = new DataTable();
             string consulta;
             consulta = "Select max(id_vtemp) from ventatemp";
             datos = buscar(consulta);
             if (datos.Rows[0][0] == DBNull.Value)
             {
                 id = 1;
        ...
        private int cod_VtempDeta()
        {
            int id;
            DataTable datos = new DataTable();
            string consulta;
            consulta = "Select max(id_vtempdeta) from ventatempdeta";
            datos = buscar(consulta);
            if (datos.Rows[0][0] == DBNull.Value)
            {
                id = 1;
        ...
        #region Venta Temporal
        public DataTable BuscaTemp()
        {
            DataTable datos = new DataTable();
            string consulta = "Select id_vtemp,Guardo,Estado,date_format(fecha,'%d/%M/%y %H;%i:%s') from ventatemp";
            return buscar(consulta);
        }

        public DataTable BuscaProdTemp(string vtemp)
        {
            DataTable datos = new DataTable();
            string consulta = "select * from ventatempdeta where id_vtemp= " + vtemp;
            return buscar(consulta);
        }



        public bool Venta_Temp(string VentaTemp, string vende, DataTable datos)
        {
            string fecha;
            fecha = DateTime.Now.ToString("yyyy/MM/dd HH:mm:ss");

            string consulta = "Update Ventatemp set Guardo='" + vende + "', Fecha='" + fecha + "', Estado='Guardado' where id_Vtemp=" + VentaTemp;

            if (consulta_gen(consulta))
            {

                return Venta_Temp_Deta(datos, VentaTemp, vende);
            }
            else
            {
                return false;
        ...
        private bool Venta_Temp_Deta(DataTable datos, string VentaTemp, string vende)
        {
            int id, total, cont;
            total = datos.Rows.Count;
            if (ElimDeta(VentaTemp))
            {
                for (cont = 0; cont < total; cont++)
                {
                    id = cod_VtempDeta();
                    ...
                    string idpord = datos.Rows[cont][0].ToString();
                    string cant = datos.Rows[cont][6].ToString();
                    consulta = "insert into ventatempdeta(id_vtempdeta,id_vtemp,id_prod,prod,estilo,tipo,color,talla,canti,precio,subtotal) " +
                                       "values(" + id + "," + VentaTemp + ",'" + datos.Rows[cont][0].ToString() + "','" + datos.Rows[cont][1].ToString() + "','" + datos.Rows[cont][2].ToString() + "','" + datos.Rows[cont][3].ToString() + "','" + datos.Rows[cont][4].ToString() + "','" + datos.Rows[cont][5].ToString() + "'," + datos.Rows[cont][6].ToString() + "," + datos.Rows[cont][7].ToString() + ","+datos.Rows[cont][8].ToString()+")";
                    if (!consulta_gen(consulta))
                    {
                        return false;
                    }
        ...
        public decimal TotalTempVen(string cod)
        {
            string consulta = "Select sum(subtotal) from ventatempdeta where id_vtemp=" + cod;
            decimal total = 0;
            DataTable datos = new DataTable();
            datos = buscar(consulta);
            if (decimal.TryParse(datos.Rows[0][0].ToString(), out total))
            {
                return total;
            }
            return total;
        ...
        public bool ElimDeta(string vent)
        {
            string consulta = "delete from ventatempdeta where id_vtemp=" + vent;
            return consulta_gen(consulta);
        }

        private bool EditVenta()
        {
            return true;
        }

        public bool Liberar(string cod)
        {
            string consulta = "update ventatemp set Estado='Libre' where id_vtemp=" + cod;
            return consulta_gen(consulta);
        }
